...
### Web UI files
  ...
  - `web_sync.py` - Builds the Trakt `/sync/history` payload from `runs/<run_id>/sync_data.json` + `picks.json`. Includes movies and episodes with `watched_at`; falls back to show-level entries when the user's pick differs from the auto-mapped TMDb id. POSTs in chunks of up to 1000 items per request. Honors a dry-run toggle.
...
## Recent Changes
- **Web sync now matches CLI completeness**: Episodes are pushed to Trakt with the original Netflix `watched_at` timestamp (not show-level only); movies also carry `watched_at`; the POST is chunked into pages of 1000 entries to avoid oversized payloads. `web_pipeline.py` writes `runs/<run_id>/sync_data.json` after the matching pass containing per-entity TMDb mappings + watched_at. `web_sync.py` rebuilds the payload from this file, applies review picks, and POSTs in chunks. When a user picks a TMDb id different from the auto-mapped one (no episode mapping available for that pick), the entry falls back to show-level entries with watched_at — surfaced in the sync UI as "Show entries". Old runs without `sync_data.json` use a legacy show-id-only fallback (also surfaced in the UI).
